...
export default {
    ...
    add: function (core) {
        ...
        /** add event listeners */
        context.image.modal.querySelector('.ke-dialog-tabs').addEventListener('click', this.openTab.bind(core));
        context.image.modal.querySelector('.ke-btn-primary').addEventListener('click', this.submit.bind(core));
    ...
    setDialog: function () {
        ...
        let html = '' +
            ...
            html += '' +
            '           <div class="ke-dialog-form">' +
            '               <label>' + lang.dialogBox.imageBox.altText + '</label><input class="ke-input-form _se_image_alt" type="text" />' +
            '           </div>';
            ...
            if (option.imageResizing) {
                ...
                '           <input class="ke-input-control _se_image_size_x" type="number" min="1" ' + (option.imageWidth === 'auto' ? 'disabled' : '') + ' /><label class="ke-dialog-size-x">x</label><input class="ke-input-control _se_image_size_y" type="number" min="1" disabled />' +
                '           <label><input type="checkbox" class="ke-dialog-btn-check _se_image_check_proportion" checked disabled/>&nbsp;' + lang.dialogBox.proportion + '</label>' +
            ...
            '               <label><input type="checkbox" class="ke-dialog-btn-check _se_image_check_caption" />&nbsp;' + lang.dialogBox.caption + '</label>' +
            '           </div>' +
            '       </div>' +
            '   </div>' +
            ...
            '       <div class="ke-dialog-body">' +
            '           <div class="ke-dialog-form">' +
            '               <label>' + lang.dialogBox.linkBox.url + '</label><input class="ke-input-form _se_image_link" type="text" />' +
            '           </div>' +
            ...
            '           <label><input type="radio" name="keditor_image_radio" class="ke-dialog-btn-radio" value="none" checked>' + lang.dialogBox.basic + '</label>' +
            '           <label><input type="radio" name="keditor_image_radio" class="ke-dialog-btn-radio" value="left">' + lang.dialogBox.left + '</label>' +
            '           <label><input type="radio" name="keditor_image_radio" class="ke-dialog-btn-radio" value="center">' + lang.dialogBox.center + '</label>' +
            '           <label><input type="radio" name="keditor_image_radio" class="ke-dialog-btn-radio" value="right">' + lang.dialogBox.right + '</label>' +
            '       </div>' +
        ...
        if (fileList.length > 0) {
            ...
            if (limitSize > 0) {
                ...
                if ((fileSize + infoSize) > limitSize) {
                    const err = '[KEDITOR.imageUpload.fail] Size of uploadable total images: ' + (limitSize/1000) + 'KB';
    ...
    onRender_imgInput: function () {
        try {
            ...
            this.closeLoading();
            throw Error('[KEDITOR.imageUpload.fail] cause : "' + e.message + '"');
        }
    },
    ...
    setup_reader: function (file, imgLinkValue, newWindowCheck, width, align, index, filesLen) {
        ...
        reader.onload = function (update, updateElement, file) {
            try {
                ...
                this.closeLoading();
                throw Error('[KEDITOR.imageFileRendering.fail] cause : "' + e.message + '"');
            }
    ...
    callBack_imgUpload: function (linkValue, linkNewWindow, width, align, update, updateElement) {
        if (this.context.image._xmlHttp.readyState === 4) {
            ...
            // error
                ...
                this.closeLoading();
                throw Error('[KEDITOR.imageUpload.fail] status: ' + this.context.image._xmlHttp.status + ', responseURL: ' + this.context.image._xmlHttp.responseURL);
            }
        }
    },
    ...
    submit: function (e) {
        ...
        contextImage._align = contextImage.modal.querySelector('input[name="keditor_image_radio"]:checked').value;
        ...
        try {
            ...
            this.closeLoading();
            throw Error('[KEDITOR.image.submit.fail] cause : "' + error.message + '"');
        } finally {
    ...
    create_image: function (src, linkValue, linkNewWindow, width, align, file) {
        ...
        const container = this.plugins.resizing.set_container.call(this, cover, 'ke-image-container');
    ...
    update_image: function (init, openController) {
        ...
        if (container === null) {
            ...
            isNewContainer = true;
            container = this.plugins.resizing.set_container.call(this, cover, 'ke-image-container');
        }
    ...
    onModifyMode: function (element, size) {
        ...
        contextImage._container = this.util.getParentElement(element, '.ke-image-container');
    ...
    openModify: function (notOpen) {
        ...
        contextImage.modal.querySelector('input[name="keditor_image_radio"][value="' + contextImage._align + '"]').checked = true;
        contextImage._align = contextImage.modal.querySelector('input[name="keditor_image_radio"]:checked').value;
    ...
    destroy: function (element) {
        ...
        const imageContainer = this.util.getParentElement(imageEl, '.ke-image-container') || imageEl;
    ...
    init: function () {
        ...
        contextImage.modal.querySelector('input[name="keditor_image_radio"][value="none"]').checked = true;
